...
func (md Metadata) Set(key string, val *string) {
	md[key] = val
}
...
func FromContext(ctx context.Context) (Metadata, bool) {
	md, ok := ctx.Value(metadataKeyZT{}).(Metadata)
	if !ok {
		return nil, ok
	}
...
func MergeContext(ctx context.Context, patchMd Metadata, overwrite bool) context.Context {
	...
	md, _ := ctx.Value(metadataKeyZT{}).(Metadata)
	cmd := make(Metadata, len(md))
	for k, v := range md {
		cmd[k] = v
	}
	...
	return context.WithValue(ctx, metadataKeyZT{}, cmd)
}
